	...
	namespace Firmware {
		...
		int GyroSensorMPU6050::connect() {
			...
			if (getGyrationResolution(&gyroResolution) == 0) 
				gyrationGain = (float)gyroResolution / std::numeric_limits<int16_t>::max();
			else
				gyrationGain = 1.0;

			AccelerationResolutions accelerationResolution;
			if (getAccelerationResolution(&accelerationResolution) == 0)
				accelerationGain = (float)accelerationResolution / std::numeric_limits<int16_t>::max();
			else
		...
		int GyroSensorMPU6050::setGyrationResolution(GyroResolutions resolution) {
			...
			if (error != 0)
				...
				gyrationGain = std::numeric_limits<int16_t>::max() / (float)resolution;
				printf("done\n");
			}
		...
		int GyroSensorMPU6050::setAccelerationResolution(AccelerationResolutions resolution) {
			...
			if (error != 0)
				...
				accelerationGain = std::numeric_limits<int16_t>::max() / (float)resolution;
				printf("done\n");
			}
